...
srs_soar_bu = LOAD ' /work/map/weeklyrefresh/pos/rd_srs_soar_bu' using PigStorage('\u0001') AS(
CreateTs:chararray,
  itm_no :chararray
      ,div_no :chararray
      ,div_nm :chararray
...
Kmt_Soar_Bu = LOAD ' /work/map/weeklyrefresh/pos/Rd_Kmt_Soar_Bu' using PigStorage('\u0001') AS(
CreateTs:chararray,
SOAR_NO :chararray
        ,SOAR_NM :chararray
...
ShcSoarBu = LOAD '/work/map/weeklyrefresh/pos/ShcSoarBu' using PigStorage('\u0001') AS
(CreateTs:chararray
...
KillVertSrs = LOAD '/work/map/weeklyrefresh/pos/KillVertSrs' using PigStorage('\u0001') AS
(CreateTs:chararray,
...
KillVertKmt = LOAD  '/work/map/weeklyrefresh/pos/KillVertKmt' using PigStorage('\u0001') AS
(CreateTs:chararray,
KillVertNm  :chararray,
 KillVertNbr  :chararray,
 KillVertSubNm  :chararray,
...
srs_year = FILTER SrsSlsPtdReq BY ToDate(TranDt,'yyyy-MM-dd HH:mm:ss')>= ToDate('2014-04-01','yyyy-MM-dd')AND ToDate(TranDt,'yyyy-MM-dd HH:mm:ss')<= ToDate('2015-03-31','yyyy-MM-dd') AND (LyltyCardNbr IS NOT NULL);
...
--KMART
kmt_year = FILTER KmtSlsPtd BY ToDate(TranDt,'yyyy-MM-dd HH:mm:ss')>= ToDate('2014-04-01','yyyy-MM-dd')AND ToDate(TranDt,'yyyy-MM-dd HH:mm:ss')<= ToDate('2015-03-31','yyyy-MM-dd') AND (LyltyCardNbr IS NOT NULL);
...
-- Primary Store
sywprimarystore = LOAD 'hdfs://heahadn01.hadoop.searshc.com:8020/work/map/weeklyrefresh/inferredinternal/SywrPrimStr' using PigStorage('\u0001') AS (datacreationts:chararray,
lyltycardnbr :chararray,
...
ShcLocn = LOAD 'hdfs://heahadn01.hadoop.searshc.com:8020/work/map/weeklyrefresh/inferredinternal/ShcLocn' using PigStorage('\u0001') AS (createTs:chararray
,LocnNbr:chararray
,StrFmt:chararray
,RgnNbr:chararray
,DistNbr:chararray
...
STORE POS_Kmt_rank_final INTO '/smith/syw_member_repo/Telluride/POS_KillVert_Rank_final_test' using PigStorage('\u0001');
