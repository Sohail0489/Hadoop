...
-- Script	: made_pos_Transformations.pig
...
--------------------------------------------------------------------------------------------------------
REGISTER '$MADE_JAR_DIR/pig_concat.jar';
REGISTER '$MADE_JAR_DIR/datafu-1.1.0.jar';
define Enumerate datafu.pig.bags.Enumerate('1');


srs_soar_bu = LOAD '$MAP_PROD_DIR/lookups/RdSrsSoarBu/current' using PigStorage('\u0001') AS(
CreateTs:chararray,
  itm_no :chararray
      ,div_no :chararray
      ,div_nm :chararray
...
Kmt_Soar_Bu = LOAD '$MAP_PROD_DIR/lookups/RdKmtSoarBu/current' using PigStorage('\u0001') AS(
CreateTs:chararray,
SOAR_NO :chararray
        ,SOAR_NM :chararray
...
SrsSlsPtd = LOAD '$MAP_PROD_DIR/pos/SywrSrsSlsPtdMstrCrm/current' using PigStorage('\u0001') AS(
CreateTs:chararray
...
KmtSlsPtd = LOAD '$MAP_PROD_DIR/pos/SywrKmtSlsPtdMstrCrm/current' using PigStorage('\u0001') AS(CreateTs:chararray
,CustIntractnId:chararray
...
ShcSoarBu = LOAD '$MAP_PROD_DIR/lookups/ShcSoarBu/current' using PigStorage('\u0001') AS
(CreateTs:chararray
...
KillVertSrs = LOAD '$MAP_PROD_DIR/lookups/KillVertSrs/current' using PigStorage('\u0001') AS
(CreateTs:chararray,
...
KillVertKmt = LOAD  '$MAP_PROD_DIR/lookups/KillVertKmt/current' using PigStorage('\u0001') AS
(CreateTs:chararray,
KillVertNm  :chararray,
 KillVertNbr  :chararray,
 KillVertSubNm  :chararray,
...
srs_year = FILTER SrsSlsPtdReq BY (LONG)YearsBetween(CurrentTime(),ToDate(TranDt,'yyyy-MM-dd HH:mm:ss'))<=1 AND (LyltyCardNbr IS NOT NULL);
...
--KMART
kmt_year = FILTER KmtSlsPtd BY (LONG)YearsBetween(CurrentTime(),ToDate(TranDt,'yyyy-MM-dd HH:mm:ss'))<=1 AND (LyltyCardNbr IS NOT NULL);
...
-- Primary Store
sywprimarystore = LOAD '$MAP_PROD_DIR/telluride/SywrPrimStr/current' using PigStorage('\u0001') AS (datacreationts:chararray,
lyltycardnbr :chararray,
...
ShcLocn = LOAD '$MAP_PROD_DIR/lookups/ShcLocn/current' using PigStorage('\u0001') AS (createTs:chararray
,LocnNbr:chararray
,StrFmt:chararray
,RgnNbr:chararray
,DistNbr:chararray
...
STORE POS_Kmt_rank_final INTO '$MADE_WORK_DIR/pos/pos_transformed' using PigStorage('\u0001');
